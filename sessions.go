...
func NewSessions(shards uint64, ttl int64, count int, deferred bool, domains Domains, new_uid_data func () Data_t) (self * Sessions_t) {
	self = &Sessions_t{}
	self.shards = shards
	for i := uint64(0); i < shards; i++ {
		self.bucket = append(self.bucket, NewSession(ttl, count, deferred, domains, new_uid_data))
	}
	return
}
