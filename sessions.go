...
func NewSessions(shards uint64, ttl int64, limit int, domains Domains, new_data NewData) (self * Sessions_t) {
	self = &Sessions_t{}
	self.shards = shards
	for i := uint64(0); i < shards; i++ {
		self.bucket = append(self.bucket, NewSession(ttl, limit, domains, new_data))
	}
	return
}
